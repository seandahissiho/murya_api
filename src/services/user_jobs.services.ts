import {PrismaClient, Quiz, UserQuiz, UserQuizStatus, UserQuizType} from '@prisma/client';
    ...
    if (!userJob) {
        ...
        if (userQuizzes.length === 0) {
            ...
            userJob = await prisma.userJob.update({
                ...
                select: {quizzes: true, completedQuizzes: true},
            });
        }
    }
...
export const saveUserQuizAnswers = async (
    ...
    return prisma.$transaction(async (tx) => {
        ...
        const allQuizzes = await tx.userQuiz.findMany({
        ...
        const maxScoreSum = allQuizzes.reduce((sum, q) => sum + q.maxScore, 0);
